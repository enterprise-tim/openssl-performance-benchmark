        ...
        <div class="card">
            ...
            <div style="background: #e7f5ff; padding: 20px; border-radius: 8px; margin-bottom: 20px; border-left: 4px solid #228be6;">
                ...
                <p style="margin-bottom: 15px; color: #495057;">
                    This benchmark measures <strong>new connection handshake throughput</strong> — how many TLS connections can be established per second. 
                    Both tests use <strong>RSA-2048 certificates</strong> for authentication, but they differ in protocol behavior:
                </p>
                
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin: 20px 0;">
                    <div style="background: white; padding: 15px; border-radius: 6px; border-top: 3px solid #868e96;">
                        <h4 style="margin-top: 0; color: #868e96; font-size: 1rem;">TLS 1.2 (Legacy)</h4>
                        <ul style="margin: 10px 0; padding-left: 25px; color: #495057; font-size: 0.95rem; line-height: 1.6;">
                            <li><strong>Cipher Suite:</strong> ECDHE-RSA-AES128-GCM-SHA256</li>
                            <li><strong>Key Exchange:</strong> Ephemeral Elliptic Curve Diffie-Hellman (ECDHE)</li>
                            <li><strong>Authentication:</strong> RSA-2048 certificate + signature</li>
                            <li><strong>Handshake:</strong> 2 round trips (4 messages)</li>
                            <li><strong>Command:</strong> <code>openssl s_time -connect localhost:4433 -cipher ECDHE-RSA-AES128-GCM-SHA256 -tls1_2 -new</code></li>
                        </ul>
                    </div>
                    
                    <div style="background: white; padding: 15px; border-radius: 6px; border-top: 3px solid #228be6;">
                        <h4 style="margin-top: 0; color: #228be6; font-size: 1rem;">TLS 1.3 (Modern)</h4>
                        <ul style="margin: 10px 0; padding-left: 25px; color: #495057; font-size: 0.95rem; line-height: 1.6;">
                            <li><strong>Cipher Suite:</strong> TLS_AES_256_GCM_SHA384 (default)</li>
                            <li><strong>Key Exchange:</strong> Integrated into handshake (secp256r1 or x25519)</li>
                            <li><strong>Authentication:</strong> RSA-2048 certificate + signature</li>
                            <li><strong>Handshake:</strong> 1 round trip (encrypted after first flight)</li>
                            <li><strong>Command:</strong> <code>openssl s_time -connect localhost:4433 -tls1_3 -new</code></li>
                        </ul>
                    </div>
                </div>
                ...
                <div style="background: #fff3bf; padding: 12px; border-radius: 6px; border-left: 3px solid #fab005; margin-top: 15px;">
                    <p style="margin: 0; color: #495057; font-size: 0.95rem;">
                        ...
                        (encrypted handshake, mandatory forward secrecy, reduced attack surface).
                    </p>
                </div>
            </div>
            
            <h3 style="color: #495057; border-bottom: 2px solid #dee2e6; padding-bottom: 8px; margin-top: 30px;">Performance Data (Connections/Second)</h3>
            <div id="tls-comparison-table" style="overflow-x: auto; margin-top: 15px;">
...
<script>
    ...
    // --- TLS COMPARISON TABLE ---
    function renderTlsComparisonTable() {
        const container = document.getElementById('tls-comparison-table');
        if (!container) return;
        
        const getTls12 = d => d.metrics.tls1_2_ecdhe_rsa_aes128gcm_cps || 0;
        const getTls13 = d => d.metrics.tls1_3_rsa_new_cps || 0;
        
        // Filter to versions with both metrics
        const tableData = data.filter(d => getTls12(d) > 0 && getTls13(d) > 0).map(d => ({
            version: d.config.version,
            tls12: getTls12(d),
            tls13: getTls13(d),
            diff: getTls13(d) - getTls12(d),
            pctChange: getTls12(d) > 0 ? ((getTls13(d) - getTls12(d)) / getTls12(d)) * 100 : 0,
            color: colorScale(getSeries(d.config.version))
        }));
        
        if (tableData.length === 0) {
            container.innerHTML = '<p style="text-align:center; color:#999; padding:20px;">No comparison data available. TLS 1.2 metrics were not captured for all versions.</p>';
            return;
        }
            ...
            <table style="width: 100%; border-collapse: collapse; font-size: 14px;">
                <thead>
                    <tr style="background: #f8f9fa; border-bottom: 2px solid #dee2e6;">
                        <th style="padding: 12px; text-align: left; font-weight: 600; color: #495057;">OpenSSL Version</th>
                        <th style="padding: 12px; text-align: right; font-weight: 600; color: #868e96;">TLS 1.2<br><span style="font-size: 0.85rem; font-weight: 400;">(ECDHE-RSA-AES128-GCM)</span></th>
                        <th style="padding: 12px; text-align: right; font-weight: 600; color: #228be6;">TLS 1.3<br><span style="font-size: 0.85rem; font-weight: 400;">(RSA-2048 Cert)</span></th>
                        <th style="padding: 12px; text-align: right; font-weight: 600; color: #495057;">Difference<br><span style="font-size: 0.85rem; font-weight: 400;">(conn/s)</span></th>
                        <th style="padding: 12px; text-align: right; font-weight: 600; color: #495057;">Change<br><span style="font-size: 0.85rem; font-weight: 400;">(%)</span></th>
                    </tr>
                </thead>
                <tbody>
        \`;
        ...
        tableData.forEach((row, idx) => {
            ...
            const changeColor = row.pctChange >= 0 ? '#40c057' : '#fa5252';
            const changeIcon = row.pctChange >= 0 ? '↑' : '↓';
            const changeSign = row.pctChange >= 0 ? '+' : '';
            
            tableHTML += \`
                <tr style="background: \${bgColor}; border-bottom: 1px solid #e9ecef;">
                    <td style="padding: 10px; font-weight: 600; color: \${row.color};">
                        \${row.version}
                    </td>
                    <td style="padding: 10px; text-align: right; font-family: 'Monaco', 'Courier New', monospace; color: #868e96;">
                        \${row.tls12.toLocaleString()} conn/s
                    </td>
                    <td style="padding: 10px; text-align: right; font-family: 'Monaco', 'Courier New', monospace; color: #228be6;">
                        \${row.tls13.toLocaleString()} conn/s
                    </td>
                    <td style="padding: 10px; text-align: right; font-family: 'Monaco', 'Courier New', monospace; color: \${changeColor};">
                        \${changeSign}\${row.diff.toLocaleString()}
                    </td>
                    <td style="padding: 10px; text-align: right; font-weight: 600; color: \${changeColor};">
                        \${changeSign}\${row.pctChange.toFixed(1)}% \${changeIcon}
                    </td>
                </tr>
            \`;
        });
        ...
        tableHTML += \`
            ...
            <div style="margin-top: 15px; padding: 12px; background: #f8f9fa; border-radius: 6px; font-size: 13px; color: #6c757d;">
                <strong>Note:</strong> Negative percentages (red ↓) indicate TLS 1.3 is <em>slower</em> than TLS 1.2. 
                Positive percentages (green ↑) indicate TLS 1.3 is <em>faster</em>. 
                All measurements use RSA-2048 certificates and represent new connection handshakes (not session resumption).
                The data shows mean values from multiple iterations to account for measurement variance.
            </div>
        \`;
    ...
    function renderForTab(tabId) {
        ...
        const run = () => {
            ...
            if (tabId === 'tls') { renderTlsChart(); renderTlsComparisonTable(); }
